help:
	...
	@echo "  deploy-dev             to deploy aauth along with dev services (rabbitmq, postgresql, ...) to docker compose for dev purposes"

SRCS = $(patsubst ./%,%,$(shell find . -name "*.go"))
GOLANGCI_LINT := $(GOPATH)/bin/golangci-lint

build: $(SRCS)
	$(GO_VARS) $(GO) build -a -installsuffix cgo -o aauth -ldflags="$(LD_FLAGS)" -tags static main.go
...
## Project Vars ##########################################################
ROOT := github.com/web-programming-fall-2022/airline-auth
DOCKER_IMAGE := ghcr.io/web-programming-fall-2022/airline-auth
...
LD_FLAGS := -X $(ROOT).Version=$(VERSION) -X $(ROOT).Commit=$(COMMIT) -X $(ROOT).BuildTime=$(BUILD_TIME) -X $(ROOT).Title=aauth
